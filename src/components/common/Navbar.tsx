  ...
  return (
    <>
      <nav
        ...
        <div className="site-container relative flex h-full items-center">
          ...
          {!hideOrderButton && (
            <Link
                href="/order"
                className="ml-auto hidden min-h-9 items-center justify-center rounded-2xl bg-brand-primary px-6 text-sm font-bold text-white no-underline shadow-[0_10px_22px_rgba(248,114,5,0.22)] transition-all duration-300 hover:-translate-y-0.5 hover:bg-brand-primary/90 hover:shadow-[0_14px_28px_rgba(248,114,5,0.28)] lg:inline-flex"
            >
              Order Now
            </Link>
          )}
          ...
          <button
            ...
            className="ml-auto flex h-9 w-9 items-center justify-center rounded-2xl text-brand-dark transition-all duration-300 hover:bg-brand-primary/10 focus:outline-none lg:hidden"
          >
      ...
      <div className={`fixed inset-0 z-[110] transition-all duration-300 lg:hidden ${isMobileMenuOpen ? "visible opacity-100" : "invisible opacity-0"}`}>
        ...
        <div
          className={`absolute bottom-0 right-0 top-0 flex w-[min(88vw,24rem)] flex-col rounded-l-[30px] bg-gradient-to-br from-white via-orange-50/70 to-brand-surface p-5 shadow-[-18px_0_50px_rgba(18,18,18,0.12)] transition-transform duration-500 min-[380px]:p-6 sm:p-8 ${
        ...
        >
          <div className="mb-8 flex items-center justify-between">
            ...
            <button
              ...
              className="flex h-8 w-8 items-center justify-center rounded-2xl bg-white/85 text-gray-500 shadow-sm transition-all duration-300 hover:bg-brand-primary/10 hover:text-brand-primary"
            >
          ...
          <nav className="flex flex-col gap-2">
            {navLinks.map((link) => (
              <Link
                ...
                href={link.href}
                onClick={() => setIsMobileMenuOpen(false)}
                className={`rounded-2xl px-4 py-2 text-lg font-semibold tracking-wide no-underline outline-none transition-all duration-300 hover:bg-brand-primary/10 hover:text-brand-primary ${
...
function NavLink({ href, active, children }: { href: string; active: boolean; children: React.ReactNode }) {
  return (
    <Link
      href={href}
      className={`relative py-1 text-base font-semibold tracking-wide no-underline outline-none transition-colors duration-300 hover:text-brand-primary hover:no-underline active:no-underline focus:outline-none focus-visible:outline-none ${
